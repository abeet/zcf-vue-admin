...
util.title = (title) => {
  title = title ? title + ' - ZCMS' : 'ZCMS-泽元内容管理系统'
  window.document.title = title
}

util.updatePriv = async () => {
  let res = await axios.get('/api/application/privs')
  Privilege.prototype.update((res && res.data && res.data.data) || {})
}
...
util.recordMenuClick = (path) => {
  if (path) {
    ...
    axios.get('/api/application/changemenuclickcount', {
      params: {
        menuPath: path
      }
    })
