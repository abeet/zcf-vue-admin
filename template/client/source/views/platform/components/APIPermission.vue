<template>
  <div class="right-list-toolbar right-list-panel-card">
    <el-toolbar>
      <el-button :loading="handlerLoading" priv="Platform.Role.SetPriv||Platform.User.SetPriv||Platform.Branch.SetPrivRange"
                 @click="saveClickHandler"><i class="fa fa-floppy-o"></i>保存</el-button>
  ...
  methods: {
    async getData () {
      ...
      if (this.id) {
        ...
        let res = await axios.get(`/api/permissions/id/${this.id}/type/${this.type}/api`)
    ...
    async saveClickHandler () {
      ...
      try {
        ...
        let res = await axios.put(`/api/permissions/id/${this.id}/type/${this.type}/api`, { data: priv })
