<template>
  <div>
    <el-toolbar>
      <tree-select class="prv-site-selector" width="180" site top :value.sync="currentSiteID" :items="siteListTreeData"></tree-select>
...
export default {
  ...
  methods: {
    ...
    async saveClickHandler () {
      ...
      try {
        let res = await axios.put(`/catalogpriv/id/${this.id}/type/${this.type}/contentprivs`, { data: updatedPermissions })
      ...
      if (this.id) {
        ...
        if (this.currentSiteID && this.siteListTreeData && this.siteListTreeData.length > 0) {
          let datas = await Promise.all([
            axios.get(`/catalogpriv/id/all/type/${this.type}/contentcolumns`),
            axios.get(`/catalogpriv/id/${this.id}/type/${this.type}/contentprivs`, { params: { 'siteID': this.currentSiteID, 'catalogID': this.currentCatalogID, dimensionID: this.dimensionID } })
          ])
          let res = datas[1]
          ...
          // 定义各列的宽度，使得文字显示比较完整
          let columnsWidth = [250, 70, 70, 70, 70, 70, 70, 70, 70, 100, 100, 100, 100, 70]
